...
impl Transport for LinuxI2c {
  ...
  fn smbus_write_block(&mut self, cmd: u8, data: &[u8]) -> Result<(), TransportError> {
    retry(|| self.dev.smbus_write_i2c_block_data(cmd, data).map_err(map_i2c_err))
  }
...
fn map_i2c_err(err: i2cdev::linux::LinuxI2CError) -> TransportError {
  match err {
    i2cdev::linux::LinuxI2CError::Errno(n) => TransportError::Io(io::Error::from_raw_os_error(n)),
