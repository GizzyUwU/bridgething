...
impl Transport for MockTransport {
    ...
    self.with_retry(|s| {
      let data = match cmd_byte {
        ...
        _ => s
          ...
          .get(&cmd_byte)
          .cloned()
          .ok_or_else(|| TransportError::Other(format!("mock: no register set for cmd 0x{:02x}", cmd_byte)))?,
      };
  ...
  fn raw_read(&mut self, out: &mut [u8]) -> Result<(), TransportError> {
    self.with_retry(|s| {
      let prepared = s
        .last_prepared
        .ok_or_else(|| TransportError::Other("mock: raw_read with no preceding prepare".into()))?;
      let data = s
        .registers
        .get(&prepared)
        .cloned()
        .ok_or_else(|| TransportError::Other(format!("mock: no register set for cmd 0x{:02x}", prepared)))?;
      if data.len() < out.len() {
        return Err(TransportError::Other(format!(
          "mock: register 0x{:02x} has {} bytes, requested {}",
          prepared,
