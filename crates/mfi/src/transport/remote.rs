...
impl Transport for RemoteI2c {
  ...
  fn smbus_read_block(&mut self, cmd: u8, out: &mut [u8]) -> Result<(), TransportError> {
    let len =
      u8::try_from(out.len()).map_err(|_| TransportError::Other(format!("smbus block len {} > 255", out.len())))?;
  ...
  fn raw_read(&mut self, out: &mut [u8]) -> Result<(), TransportError> {
    let len =
      u32::try_from(out.len()).map_err(|_| TransportError::Other(format!("raw_read len {} > u32::MAX", out.len())))?;
...
fn handle_request<T: Transport>(transport: &mut T, tag_byte: u8, payload: &[u8]) -> Result<Vec<u8>, String> {
  match tag_byte {
    tag::PREPARE => {
      let cmd = *payload.first().ok_or_else(|| "PREPARE: missing cmd byte".to_string())?;
      transport.prepare(cmd).map_err(|e| e.to_string())?;
      Ok(Vec::new())
    }
    tag::SMBUS_READ => {
      if payload.len() != 2 {
        return Err(format!("SMBUS_READ: expected 2 bytes, got {}", payload.len()));
      }
      let cmd = payload[0];
      let len = payload[1] as usize;
      let mut buf = vec![0u8; len];
      transport.smbus_read_block(cmd, &mut buf).map_err(|e| e.to_string())?;
      Ok(buf)
    }
    tag::SMBUS_WRITE => {
      let cmd = *payload
    ...
    tag::RAW_READ => {
      if payload.len() != 4 {
        return Err(format!("RAW_READ: expected 4-byte length, got {}", payload.len()));
      }
