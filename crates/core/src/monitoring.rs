pub fn init_logger(tap: crate::state::LogTapLayer) {
  ...
  let filter_directives = if let Ok(filter) = std::env::var("RUST_LOG") {
    ...
    "bridgething=trace,bridgething::ws::connection::send=debug,bridgething::net=debug,libbridgething=trace,bridgething-iap2=trace,bridgething-mfi=trace".to_string()
  };
