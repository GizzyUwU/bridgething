...
struct FakeMediaBackend {
  granted: Mutex<bool>,
  sessions: Mutex<Vec<MediaSessionSnapshot>>,
  art: Mutex<std::collections::HashMap<(String, String), MediaArt>>,
  controls: Mutex<Vec<(String, MediaControl)>>,
  inbox: Mutex<Option<Arc<MediaSessionInbox>>>,
}
...
impl FakeMediaBackend {
  ...
  fn emit(&self, sessions: Vec<MediaSessionSnapshot>) {
  ...
  fn controls(&self) -> Vec<(String, MediaControl)> {
    self.controls.lock().unwrap().clone()
  }

  async fn wait_control(&self, wanted: MediaControl) {
...
fn playing(title: &str, artist: &str, package: &str) -> MediaSessionSnapshot {
