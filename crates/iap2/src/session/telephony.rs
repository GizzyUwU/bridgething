...
use crate::{
  csm::{
    CsmFrame,
    telephony::{
      AcceptCall, CallStateUpdate, CommunicationsUpdate, EndCall, HoldStatusUpdate, InitiateCall, MergeCalls,
      MuteStatusUpdate, SendDtmf, StartCallStateUpdates, StartCommunicationsUpdates, SwapCalls,
    },
  },
  error::Result,
  link::Iap2Command,
...
impl TelephonyFlow {
  ...
  pub(super) async fn handle(
    ...
    let event = match frame.msg_id {
      CallStateUpdate::CSM_MSG_ID => SessionEvent::CallStateUpdate(CallStateUpdate::try_from(frame)?),
      CommunicationsUpdate::CSM_MSG_ID => SessionEvent::CommunicationsUpdate(CommunicationsUpdate::try_from(frame)?),
      _ => return Ok(None),
    };
  ...
  pub(super) async fn handle_command(
    ...
    if !matches!(self.state, SubscriptionState::Subscribed) {
      tracing::warn!(?cmd, "iap2 telephony: command before subscription; dropping");
      return Ok(());
    }
