...
async fn write_packet<W: AsyncWrite + Unpin>(writer: &mut W, codec: &mut LinkCodec, packet: LinkPacket) {
...
async fn handshake_reaches_established_and_peer_lsp_propagates() {
  ...
  assert!(
    matches!(result, Err(bridgething_iap2::Error::PeerDisconnected)),
...
async fn disconnect_command_sends_rst_and_returns_ok() {
  ...
  let result = tokio::time::timeout(Duration::from_secs(2), link_handle)
    ...
    .expect("link task panicked");
  assert!(result.is_ok(), "expected Ok on local disconnect, got {:?}", result);
}
