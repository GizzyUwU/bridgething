...
async fn find_our_characteristic(device: &Device) -> bluer::Result<Option<Characteristic>> {
  ...
  if uuids.contains(&BRIDGETHING_SERVICE_UUID) {
    ...
    for service in device.services().await? {
      ...
      if uuid == BRIDGETHING_SERVICE_UUID {
        ...
        for char in service.characteristics().await? {
          ...
          if uuid == BRIDGETHING_CHARACTERISTIC_UUID {
...
pub async fn scan_and_connect() -> anyhow::Result<()> {
