...
// frame header + msgpack body) so it works without a checkout of the
...
//
// Usage:
//   bun run push                           # uses bridgething.local
//   bun run push <host>                    # explicit host or IP
//   bun run push --skip-build [host]
//   bun run push --no-switch  [host]
//
// Env:
...
//   BRIDGETHING_BUNDLE_NAME       remote dir name under /var/bridgething/webapps/
//                                 (default: the manifest's id)
...
// the `trusted` zone or pass the IP directly.
...
async function sendSwitchTo(host: string, port: number, manifestId: string): Promise<SwitchOutcome> {
  ...
  // Default-shaped GatewayCapabilities. The daemon's deserializer needs
  // every required field present; serde drops Option::None, so anything
  // not here defaults to its zero-value on the daemon side.
