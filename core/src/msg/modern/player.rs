...
use crate::{
  dbus::{PlayerRepeat, PlayerShuffle, PlayerState, PlayerStatus, PlayerTrack},
  msg::SendMsgData,
};
...
impl PlayerSend {
  pub fn state_from_player_state(state: &PlayerState) -> Self {
    Self::SpotifyPlayerState {
      context_uri: "spotify:context:fake".to_string(),
      context_title: "BridgeThing".to_string(),
      is_paused: state.status == PlayerStatus::Paused,
      is_paused_bool: state.status == PlayerStatus::Paused,
      playback_options: PlaybackOptions {
        repeat: if state.repeat == PlayerRepeat::On { 1 } else { 0 },
        shuffle: state.shuffle == PlayerShuffle::On,
      },
      playback_position: state.position,
      playback_restrictions: PlaybackRestrictions {
      ...
      playback_speed: 1.0,
      track: state.track.clone().into(),
    }
  }

  pub fn queue_from_player_state(state: &PlayerState) -> Self {
    Self::PlayerQueue {
      next: vec![],
      current: state.track.clone().into(),
      previous: vec![],
    }
  }
...
impl From<PlayerSend> for SendMsgData {
  fn from(val: PlayerSend) -> Self {
    Self::Player(val)
  }
}
...
impl From<PlayerTrack> for Track {
  fn from(track: PlayerTrack) -> Self {
    Self {
      name: track.title,
      artist: track.artists.get(0).cloned().unwrap_or_default().into(),
      artists: track.artists.into_iter().map(Into::into).collect(),
      duration_ms: track.duration,
      image_id: format!("spotify:image:{}", to_spotify_slug(&track.album)),
      is_episode: false,
      is_podcast: false,
      saved: false,
      uid: to_spotify_slug(&track.album),
      uri: format!("spotify:context:{}", to_spotify_slug(&track.album)),
...
impl From<PlayerTrack> for QueueTrack {
  fn from(track: PlayerTrack) -> Self {
    Self {
      uid: to_spotify_slug(&track.album),
      uri: format!("spotify:track:{}", to_spotify_slug(&track.title)),
      name: track.title,
      artists: track.artists.into_iter().map(Into::into).collect(),
      image_uri: format!("spotify:image:{}", to_spotify_slug(&track.album)),
      provider: "context".to_string(),
    }
  }
}
...
impl From<String> for Album {
  fn from(name: String) -> Self {
    Self {
      uri: format!("spotify:album:{}", to_spotify_slug(&name)),
      name,
    }
  }
}
...
impl From<String> for Artist {
  fn from(name: String) -> Self {
    Self {
      uri: format!("spotify:artist:{}", to_spotify_slug(&name)),
      name,
    }
  }
}
...
pub struct PlaybackRestrictions {
  can_repeat_context: bool,
  can_repeat_track: bool,
  can_seek: bool,
  can_skip_next: bool,
  can_skip_prev: bool,
  can_toggle_shuffle: bool,
}

pub fn to_spotify_slug(value: &str) -> String {
