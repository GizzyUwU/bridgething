...
const TEST_SERIAL: [u8; SERIAL_LEN] = [
  0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF, 0x10, 0x32, 0x54, 0x76, 0x98, 0xBA, 0xDC, 0xFE, 0xFE, 0xDC, 0xBA,
  0x98, 0x76, 0x54, 0x32, 0x10, 0xEF, 0xCD, 0xAB, 0x89, 0x67, 0x45, 0x23, 0x01,
];
...
fn cert_settles_before_reading_payload() {
  ...
  let settles = sleeps.iter().filter(|d| **d == Duration::from_millis(10)).count();
  assert_eq!(
    settles, 2,
...
fn sign_writes_challenge_to_chip() {
  ...
  let stored = mock.shared().borrow().registers.get(&0x21).cloned().unwrap();
...
fn sign_observes_500ms_poll_delay() {
  ...
  assert!(
    state.sleeps.iter().any(|d| *d == Duration::from_millis(500)),
